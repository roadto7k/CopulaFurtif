...
from CopulaFurtif.core.copulas.domain.estimation.estimation import (
    pseudo_obs as _pseudo_obs_pair,
    _robust_init_from_uv,
)
...
# Public API
# -----------------------------------------------------------------------------
def _subsample_pair_for_cmle(
    x: np.ndarray,
    y: np.ndarray,
    max_obs: Optional[int],
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Subsample a pair deterministically for the CMLE refinement stage only.
    Keeps the full data for final diagnostics / AIC recomputation.
    """
    x = np.asarray(x, dtype=float).ravel()
    y = np.asarray(y, dtype=float).ravel()

    if max_obs is None or max_obs <= 0 or len(x) <= int(max_obs):
        return x, y

    idx = np.linspace(0, len(x) - 1, int(max_obs), dtype=int)
    return x[idx], y[idx]
...
def fit_copulas(
    ...
    *,
    ...
    eps_pdf: float = 1e-12,
    inputs_are_uniform: bool = False,
    cmle_max_obs: Optional[int] = None,
    ...
    for candidate in all_candidates:
        ...
        try:
            ...
            if inputs_are_uniform:
                bounds = (
                    cop.get_bounds()
                    if hasattr(cop, "get_bounds")
                    else None
                )

                init_guess = _robust_init_from_uv(
                    cop,
                    np.asarray(
                        x,
                        dtype=float,
    ...
    if refine_enabled:
        ...
        # Select Stage 2 candidates using the same criterion as the
        # requested final selection mode.
        selection_key = str(
            selection
        ).lower().strip()

        if selection_key == "aic":
            refinement_metric = "aic"
        else:
            refinement_metric = "score_stage1"

        df_ok_sorted = df_ok.sort_values(
            refinement_metric,
            kind="mergesort",
        ).reset_index(drop=True)

        refine_names = df_ok_sorted.loc[
                       : k - 1,
                       "name",
                       ].tolist()
        ...
        for nm in refine_names:
            ...
            try:
                ...
                # Copula-likelihood refinement.
                #
                # If x and y are already PIT values, optimize directly on those PITs.
                # Otherwise, let CMLE build empirical rank pseudo-observations.
                # quick CMLE refine on a subsample only
                x_refine, y_refine = _subsample_pair_for_cmle(
                    x,
                    y,
                    cmle_max_obs,
                )

                fit_result = fitter.fit_cmle(
                    (x_refine, y_refine),
                    copula=cop,
                    quick=True,
                    verbose=False,
                ...
                if fit_result is None:
                    raise RuntimeError(
                        f"Copula refinement failed for '{nm}'."
                    )
