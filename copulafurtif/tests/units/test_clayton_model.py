...
def test_parameters_set_get(copula):
    ...
    assert np.allclose(copula.parameters, [2.0])

def test_parameters_out_of_bounds(copula):
    with pytest.raises(ValueError):
        copula.parameters = [0.0]  # Out of lower bound
    with pytest.raises(ValueError):
        copula.parameters = [50.0]  # Out of upper bound
...
def test_partial_derivatives_and_conditionals(copula):
    ...
    cond_u_v = copula.conditional_cdf_u_given_v(u, v)
    cond_v_u = copula.conditional_cdf_v_given_u(u, v)
    assert np.isclose(cond_u_v, dv)
    assert np.isclose(cond_v_u, du)
