    ...
    while True:
        ...
        if portfolio_stopped:
            ...
            if len(accepted) < 2 or df_trade.empty:
                ...
                rank_df = rank_coins_by_kendall_returns(
                    prices=df_form,
                    ref=p.ref,
                    coins=accepted,
                    min_obs=30,
                ...
                top = rank_df["coin"].tolist()[:2] if not rank_df.empty else []

                if len(top) < 2:
                    skip_status = "SKIP (ranking failed)"
                else:
                    coin1, coin2 = top[0], top[1]
                    tau1 = float(rank_df.loc[rank_df["coin"] == coin1, "tau"].iloc[0])
                    tau2 = float(rank_df.loc[rank_df["coin"] == coin2, "tau"].iloc[0])
        ...
        weekly.append(dict(
            ...
            tau1=float(tau1) if np.isfinite(tau1) else np.nan,
            tau2=float(tau2) if np.isfinite(tau2) else np.nan,
            ranking_method="kendall_log_returns_vs_ref",
            q1=q1, q2=q2,
