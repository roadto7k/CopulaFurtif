# dash_bot/core/copula_engine.py
import numpy as np
from typing import Any, Tuple, Optional, List
import pandas as pd
import scipy.stats as stats
from CopulaFurtif.copulas import CopulaFactory, CopulaType
from DataAnalysis.Utils.copulas import fit_copulas
from CopulaFurtif.core.copulas.adapters import RotatedCopula
import re

HAS_COPULAFURTIF = True

def _get_all_copula_values():
    if HAS_COPULAFURTIF and CopulaType is not None:
        return [ct.value for ct in list(CopulaType)]
    return ["gaussian", "student", "clayton", "frank", "gumbel"]

def _normalize_copula_name(name: str) -> str:
    """
    Normalize a copula name for robust matching.

    Examples
    --------
    "Student-t"  -> "studentt"
    "TAWN T1"    -> "tawnt1"
    "BB1"        -> "bb1"
    """
    return re.sub(
        r"[^a-z0-9]+",
        "",
        str(name).lower(),
    )


def _copula_type_from_value(val: str):
    """
    Resolve a CopulaType from a dashboard / fitting name.
    """
    if not (
        HAS_COPULAFURTIF
        and CopulaType is not None
    ):
        return None

    key = _normalize_copula_name(val)

    aliases = {
        "t": CopulaType.STUDENT,
        "studentt": CopulaType.STUDENT,
        "studenttcopula": CopulaType.STUDENT,
        "normal": CopulaType.GAUSSIAN,
        "gauss": CopulaType.GAUSSIAN,
    }

    if key in aliases:
        return aliases[key]

    for ct in CopulaType:
        if key in (
            _normalize_copula_name(ct.value),
            _normalize_copula_name(ct.name),
        ):
            return ct

    return None


def _parse_rotation_suffix(name: str) -> Tuple[str, int]:
    """
    Parse a copula name carrying an optional rotation suffix.

    Examples
    --------
    "Clayton R90"   -> ("Clayton", 90)
    "TAWN T1 R270"  -> ("TAWN T1", 270)
    "Gaussian"      -> ("Gaussian", 0)
    """
    nm = str(name).strip()

    match = re.search(
        r"\s+R(90|180|270)\s*$",
        nm,
        flags=re.IGNORECASE,
    )

    if match is None:
        return nm, 0

    base_name = nm[:match.start()].strip()
    rotation = int(match.group(1))

    return base_name, rotation


def build_copula(name: str, params: Any):
    """
    Rebuild exactly the copula fitted by CopulaFurtif.

    Trading code must never alter fitted copula parameters.
    Parameter validation is delegated to CopulaFurtif.
    """
    if not HAS_COPULAFURTIF or CopulaFactory is None:
        raise RuntimeError(
            "CopulaFurtif non disponible."
        )

    nm_raw = str(name).strip()

    base_name, rotation = _parse_rotation_suffix(
        nm_raw
    )

    ct = _copula_type_from_value(
        base_name
    )

    if ct is None:
        raise ValueError(
            f"CopulaType introuvable pour '{base_name}' "
            f"(nom original: '{nm_raw}')"
        )

    if ct == CopulaType.TAWN:
        raise ValueError(
            "The generic 3-parameter Tawn copula is not enabled. "
            "TAWN T1 and TAWN T2 are supported."
        )

    p = np.asarray(
        params,
        dtype=float,
    ).ravel()

    if p.size == 0:
        raise ValueError(
            f"Empty fitted parameters for '{nm_raw}'."
        )

    if not np.all(np.isfinite(p)):
        raise ValueError(
            f"Invalid fitted parameters for '{nm_raw}': "
            f"{p.tolist()}"
        )

    base_cop = CopulaFactory.create(ct)

    try:
        base_cop.set_parameters(
            p.tolist()
        )

    except Exception as exc:
        raise ValueError(
            f"Unable to rebuild fitted copula '{nm_raw}'. "
            f"params={p.tolist()}, "
            f"parameter_names={base_cop.get_parameters_names()}, "
            f"bounds={base_cop.get_bounds()}"
        ) from exc

    if rotation != 0:
        return RotatedCopula(
            base_cop,
            rotation,
        )

    return base_cop

def _call_cdf(cop, u: float, v: float) -> float:
    """
    Évalue C(u,v) via CopulaFurtif.
    """
    u = float(np.clip(u, 1e-10, 1 - 1e-10))
    v = float(np.clip(v, 1e-10, 1 - 1e-10))

    def _as_float(out):
        return float(np.asarray(out).ravel()[0])

    # CopulaFurtif API : get_cdf(u, v)
    if hasattr(cop, "get_cdf"):
        try:
            return _as_float(cop.get_cdf(u, v))
        except Exception:
            pass

    # Variantes d'API CopulaFurtif
    for attr in ("get_CDF", "cdf", "CDF"):
        if hasattr(cop, attr):
            try:
                fn = getattr(cop, attr)
                return _as_float(fn(u, v))
            except Exception:
                pass

    # Dernier recours : objet callable
    if callable(cop):
        try:
            return _as_float(cop(u, v))
        except Exception:
            pass

    raise RuntimeError("Impossible d'évaluer la CDF du copula via CopulaFurtif.")

def copula_h_funcs(cop, u: float, v: float, eps: float = 1e-4) -> Tuple[float, float]:
    """
    h1|2 = P(U<=u | V=v), h2|1 = P(V<=v | U=u)
    - Si la copule CopulaFurtif fournit des conditionnelles built-in -> on les utilise
    - Sinon -> dérivées numériques via CDF
    """
    u = float(np.clip(u, eps, 1 - eps))
    v = float(np.clip(v, eps, 1 - eps))

    # 1) Built-in conditionnelles CopulaFurtif
    if hasattr(cop, "conditional_cdf_u_given_v") and hasattr(cop, "conditional_cdf_v_given_u"):
        try:
            h12 = float(cop.conditional_cdf_u_given_v(u, v))
            h21 = float(cop.conditional_cdf_v_given_u(u, v))
            return float(np.clip(h12, 0.0, 1.0)), float(np.clip(h21, 0.0, 1.0))
        except Exception:
            pass

    # 2) Dérivées numériques via la CDF
    dv = min(eps, v - 1e-8, 1 - 1e-8 - v)
    du = min(eps, u - 1e-8, 1 - 1e-8 - u)
    dv = max(dv, 1e-6)
    du = max(du, 1e-6)

    c_up = _call_cdf(cop, u, min(v + dv, 1 - 1e-8))
    c_dn = _call_cdf(cop, u, max(v - dv, 1e-8))
    h12 = (c_up - c_dn) / (2.0 * dv)

    c_up2 = _call_cdf(cop, min(u + du, 1 - 1e-8), v)
    c_dn2 = _call_cdf(cop, max(u - du, 1e-8), v)
    h21 = (c_up2 - c_dn2) / (2.0 * du)

    return float(np.clip(h12, 0.0, 1.0)), float(np.clip(h21, 0.0, 1.0))

def ecdf_value(sorted_x: np.ndarray, x: float) -> float:
    """
    Empirical CDF (CML style): F_hat(x) = rank/(n+1).
    """
    n = len(sorted_x)
    if n == 0:
        return np.nan
    # number <= x
    k = int(np.searchsorted(sorted_x, x, side="right"))
    return float(np.clip(k / (n + 1.0), 1e-6, 1 - 1e-6))



def _fit_best_marginal(x: np.ndarray) -> dict:
    """Fit one marginal among Gaussian / Student-t / Cauchy by AIC."""
    x = np.asarray(x, dtype=float).ravel()
    x = x[np.isfinite(x)]
    candidates = [
        ("norm", stats.norm),
        ("t", stats.t),
        ("cauchy", stats.cauchy),
    ]
    best = None
    best_aic = np.inf
    for name, dist in candidates:
        try:
            params = dist.fit(x)
            ll = float(np.sum(dist.logpdf(x, *params)))
            k = len(params)
            aic = 2.0 * k - 2.0 * ll
            if np.isfinite(aic) and aic < best_aic:
                best_aic = aic
                best = {
                    "name": name,
                    "dist": dist,
                    "params": tuple(float(p) for p in params),
                    "aic": float(aic),
                    "loglik": ll,
                }
        except Exception:
            continue
    if best is None:
        params = stats.norm.fit(x)
        best = {
            "name": "norm",
            "dist": stats.norm,
            "params": tuple(float(p) for p in params),
            "aic": np.nan,
            "loglik": np.nan,
        }
    return best


def marginal_cdf_value(marginal: dict, x: float, eps: float = 1e-6) -> float:
    """CDF under a fitted marginal, clipped to the open unit interval."""
    dist = marginal["dist"]
    params = marginal["params"]
    return float(np.clip(dist.cdf(float(x), *params), eps, 1.0 - eps))


def marginal_cdf_array(marginal: dict, x: np.ndarray, eps: float = 1e-6) -> np.ndarray:
    """Vectorized CDF under a fitted marginal, clipped to the open unit interval."""
    dist = marginal["dist"]
    params = marginal["params"]
    return np.clip(np.asarray(dist.cdf(np.asarray(x, dtype=float), *params), dtype=float), eps, 1.0 - eps)


def _serialize_marginal(m: dict) -> dict:
    return {
        "name": str(m.get("name", "?")),
        "params": [float(p) for p in tuple(m.get("params", ()))],
        "aic": float(m.get("aic", np.nan)) if np.isfinite(m.get("aic", np.nan)) else None,
        "loglik": float(m.get("loglik", np.nan)) if np.isfinite(m.get("loglik", np.nan)) else None,
    }

def is_copula_evaluable(name: str, params: Any) -> bool:
    """Teste si on peut calculer h-functions (donc trading) avec ce copula."""
    try:
        cop = build_copula(name, params)
        h12, h21 = copula_h_funcs(cop, 0.37, 0.61)
        return bool(np.isfinite(h12) and np.isfinite(h21))
    except Exception:
        return False

def fit_pair_copula(
    s1: pd.Series,
    s2: pd.Series,
    pick_mode: str,
    manual_name: str,
    suppress_logs: bool = True,
) -> Tuple[Optional[str], Optional[Any], pd.DataFrame, List[str]]:
    """
    Fit copulas on parametric PIT-transformed spreads (paper-style marginals).
    Returns (best_name, best_params, df_fit, messages). The fitted marginals are stored in
    df_fit.attrs["marginal1"] / df_fit.attrs["marginal2"].
    """
    s1, s2 = s1.align(s2, join="inner")
    if len(s1) < 50:
        return None, None, pd.DataFrame(), ["Pas assez d'obs pour fitter le copula."]
    x = s1.to_numpy(dtype=float)
    y = s2.to_numpy(dtype=float)

    # Fit marginal distributions on formation spreads, then PIT-transform to (u,v)
    m1 = _fit_best_marginal(x)
    m2 = _fit_best_marginal(y)
    u = marginal_cdf_array(m1, x)
    v = marginal_cdf_array(m2, y)

    # Decide selection logic (fast/medium) based on the dashboard choice
    pick = str(pick_mode or "").lower().strip()
    if pick == "best_aic":
        # Fast production mode:
        # rank by AIC, but only refine the most promising candidates.
        fit_kwargs = dict(
            selection="aic",
            include_pit=False,
            refine_topk=5,
            cmle_max_obs=10_000,
        )
    elif pick == "best_score_pit":
        # Most expensive option: adds Rosenblatt-PIT and refines top candidates with a quick CMLE.
        fit_kwargs = dict(selection="score", include_pit=True, pit_m=250, refine_topk=2)
    else:
        # Default: robust score using AIC + Kendall-tau error + tail mismatch (no PIT).
        fit_kwargs = dict(selection="score", include_pit=False, refine_topk=0)

    # Fit (optionally silence logs to avoid CMLE/diagnostic spam)
    import io, contextlib
    if suppress_logs:
        _buf = io.StringIO()
        with contextlib.redirect_stdout(_buf), contextlib.redirect_stderr(_buf):
            df_fit, msgs = fit_copulas(u, v, inputs_are_uniform=True, **fit_kwargs)
        _fit_log = _buf.getvalue().strip()
        # keep captured logs only if nothing else is reported
        if (not msgs) and _fit_log:
            msgs = [_fit_log]
    else:
        df_fit, msgs = fit_copulas(u, v, inputs_are_uniform=True, **fit_kwargs)

    if df_fit is None or df_fit.empty:
        return None, None, pd.DataFrame(), (msgs or ["fit_copulas: aucun résultat."])

    df_fit.attrs["marginal1"] = m1
    df_fit.attrs["marginal2"] = m2
    df_fit.attrs["marginal1_serialized"] = _serialize_marginal(m1)
    df_fit.attrs["marginal2_serialized"] = _serialize_marginal(m2)

    # Mark copulas that can be evaluated by the trading engine.
    if "evaluable" not in df_fit.columns:
        df_fit = df_fit.copy()

        params_col = (
            "params_effective"
            if "params_effective" in df_fit.columns
            else "params"
        )

        df_fit["evaluable"] = [
            is_copula_evaluable(
                r["name"],
                r[params_col],
            )
            for _, r in df_fit.iterrows()
        ]

    if pick == "manual":
        target = str(manual_name).lower()
        row = df_fit[df_fit["name"].astype(str).str.lower() == target]
        if row.empty:
            msgs = (msgs or []) + [f"Copula '{manual_name}' introuvable dans le fit => fallback meilleur modèle évaluable."]
        else:
            cand = row.iloc[0]
            if bool(cand.get("evaluable", True)):
                params_out = cand.get("params_effective", cand.get("params", None))
                return str(cand["name"]), params_out, df_fit, msgs
            msgs = (msgs or []) + [f"Copula '{manual_name}' non-évaluable (CDF/h-functions indisponibles) => fallback."]

    # pick best evaluable
    df_ok = df_fit[df_fit.get("evaluable", True) == True]  # noqa: E712
    if df_ok.empty:
        return None, None, df_fit, (msgs or []) + ["Aucune copula évaluable pour le trading (CDF/h-functions indisponibles). "
                                                  "Choisis une famille supportée (Gaussian/Student/Clayton/Frank/Gumbel) "
                                                  "ou étends _call_cdf pour CopulaFurtif."]
    best = df_ok.iloc[0]
    params_out = best.get("params_effective", best.get("params", None))
    return str(best["name"]), params_out, df_fit, msgs


