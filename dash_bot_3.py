...
def backtest_reference_copula(prices: pd.DataFrame, p: BacktestParams) -> Dict[str, Any]:
    ...
    while True:
        ...
        # compute pseudo-observations for viz (CML: rank/(n+1))
        _s1_vals = s1_form.dropna().values.astype(float)
        _s2_vals = s2_form.dropna().values.astype(float)
        _s1_aligned, _s2_aligned = s1_form.align(s2_form, join="inner")
        _s1_aligned = _s1_aligned.dropna()
        _s2_aligned = _s2_aligned.loc[_s1_aligned.index].dropna()
        _common_idx = _s1_aligned.index.intersection(_s2_aligned.index)
        _s1a = _s1_aligned.loc[_common_idx].values.astype(float)
        _s2a = _s2_aligned.loc[_common_idx].values.astype(float)
        if len(_s1a) > 10:
            _n = len(_s1a)
            pseudo_u = (pd.Series(_s1a).rank().values / (_n + 1.0)).tolist()
            pseudo_v = (pd.Series(_s2a).rank().values / (_n + 1.0)).tolist()
        else:
            pseudo_u = []
            pseudo_v = []
        ...
        weekly.append(dict(
            ...
            copula=best_name,
            copula_params=best_params.tolist() if hasattr(best_params, 'tolist') else list(best_params),
            pseudo_obs_u=pseudo_u if isinstance(pseudo_u, list) else list(pseudo_u),
            pseudo_obs_v=pseudo_v if isinstance(pseudo_v, list) else list(pseudo_v),
...
# -----------------------------------------------------------------------------
# Copula diagnostic visualizations (Tron-styled Plotly)
# -----------------------------------------------------------------------------

def fig_copula_pdf_contour(cop_name: str, cop_params, pseudo_u, pseudo_v, nsplit: int = 60) -> go.Figure:
    """PDF iso-contour plot with pseudo-observations scatter overlay."""
    fig = go.Figure()

    has_cop = False
    try:
        cop = build_copula(cop_name, cop_params)
        has_cop = True
    except Exception:
        pass
    ...
    if has_cop:
        ...
        for i in range(nsplit):
            for j in range(nsplit):
                try:
                    z = cop.get_pdf(U[i, j], V[i, j])
                    Z[i, j] = float(z) if np.isfinite(z) else 0.0
                except Exception:
                    Z[i, j] = 0.0
        ...
        # Tron-style colorscale
        tron_pdf_cs = [
            [0.0, "rgba(6,11,20,0.9)"],
            [0.15, "rgba(0,120,130,0.4)"],
            [0.35, "rgba(0,200,210,0.5)"],
            [0.55, "rgba(0,240,255,0.6)"],
            [0.75, "rgba(180,60,230,0.7)"],
            [1.0, "rgba(255,46,237,0.9)"],
        ]

        fig.add_trace(go.Contour(
            x=grid, y=grid, z=Z_log,
            colorscale=tron_pdf_cs,
            contours=dict(
                coloring="heatmap",
                showlabels=False,
                start=float(np.percentile(Z_log[Z_log > 0], 5)) if np.any(Z_log > 0) else 0,
                end=float(np.percentile(Z_log[Z_log > 0], 98)) if np.any(Z_log > 0) else 1,
                size=float((np.percentile(Z_log[Z_log > 0], 98) - np.percentile(Z_log[Z_log > 0], 5)) / 12) if np.any(Z_log > 0) else 0.1,
            ),
            line=dict(width=0.5, color="rgba(0,240,255,0.3)"),
            colorbar=dict(
                title=dict(text="log(1+c)", font=dict(family="Orbitron", size=9, color="#5a7a90")),
                tickfont=dict(family="Share Tech Mono", size=8, color="#5a7a90"),
                outlinecolor="rgba(0,240,255,0.1)", outlinewidth=1, len=0.7,
            ),
            name="PDF density",
        ))

    # Scatter pseudo-observations
    if pseudo_u and pseudo_v and len(pseudo_u) > 0:
        fig.add_trace(go.Scatter(
            x=pseudo_u, y=pseudo_v,
            mode="markers",
            marker=dict(size=3, color="#00ff88", opacity=0.5, line=dict(width=0)),
            name="Pseudo-obs (formation)",
        ))

    pair_label = cop_name or "N/A"
    fig.update_layout(
        **TRON_LAYOUT, height=420,
        title=f"⬡  COPULA PDF ISO-CONTOURS — {pair_label.upper()}",
        xaxis_title="u (spread 1)", yaxis_title="v (spread 2)",
        xaxis=dict(**TRON_LAYOUT["xaxis"], range=[0, 1]),
        yaxis=dict(**TRON_LAYOUT["yaxis"], range=[0, 1], scaleanchor="x"),
    )
    return fig


def fig_dependency_scatter(pseudo_u, pseudo_v, cop_name: str, cop_params=None) -> go.Figure:
    """Dependency scatter with h-function conditional probability coloring."""
    fig = go.Figure()

    if not pseudo_u or not pseudo_v or len(pseudo_u) < 10:
        return fig_empty("Dependency scatter — not enough data")

    u_arr = np.array(pseudo_u, dtype=float)
    v_arr = np.array(pseudo_v, dtype=float)

    # Try to compute h12 = P(U<=u|V=v) for coloring
    h_colors = None
    try:
        cop = build_copula(cop_name, cop_params)
        h_vals = []
        for ui, vi in zip(u_arr, v_arr):
            try:
                h12 = float(cop.conditional_cdf_u_given_v(
                    np.clip(ui, 1e-6, 1-1e-6), np.clip(vi, 1e-6, 1-1e-6)
                ))
                h_vals.append(h12)
            except Exception:
                h_vals.append(0.5)
        h_colors = np.array(h_vals)
    except Exception:
        pass

    if h_colors is not None:
        h_cs = [
            [0.0, "#ff3355"],    # extreme low
            [0.15, "#ff6f1a"],
            [0.4, "#0a1628"],    # neutral zone
            [0.6, "#0a1628"],
            [0.85, "#00b8cc"],
            [1.0, "#00f0ff"],    # extreme high
        ]
        fig.add_trace(go.Scatter(
            x=u_arr.tolist(), y=v_arr.tolist(),
            mode="markers",
            marker=dict(
                size=5, color=h_colors.tolist(), colorscale=h_cs,
                cmin=0, cmax=1, opacity=0.7,
                colorbar=dict(
                    title=dict(text="h(u|v)", font=dict(family="Orbitron", size=9, color="#5a7a90")),
                    tickfont=dict(family="Share Tech Mono", size=8, color="#5a7a90"),
                    outlinecolor="rgba(0,240,255,0.1)", outlinewidth=1, len=0.6, x=1.02,
                ),
                line=dict(width=0),
            ),
            name="Pseudo-obs",
        ...
        fig.add_trace(go.Scatter(
            x=u_arr.tolist(), y=v_arr.tolist(),
            mode="markers",
            marker=dict(size=4, color="#00f0ff", opacity=0.5),
            name="Pseudo-obs",
        ))
    ...
    fig.update_layout(
        **TRON_LAYOUT, height=420,
        title=f"⬡  DEPENDENCY SCATTER — {cop_name.upper() if cop_name else 'N/A'}",
        xaxis_title="u (spread 1)", yaxis_title="v (spread 2)",
        xaxis=dict(**TRON_LAYOUT["xaxis"], range=[0, 1]),
        yaxis=dict(**TRON_LAYOUT["yaxis"], range=[0, 1], scaleanchor="x"),
    )
...
def fig_arbitrage_frontiers(cop_name: str, cop_params, pseudo_u, pseudo_v,
                            alpha_low: float = 0.05, alpha_high: float = 0.95,
                            n_grid: int = 120) -> go.Figure:
    """Conditional quantile frontiers (arbitrage detection) with data overlay."""
    fig = go.Figure()
    ...
    if has_cop:
        ...
        for v in v_grid:
            try:
                c_lo = float(cop.conditional_cdf_u_given_v(eps, v))
                c_hi = float(cop.conditional_cdf_u_given_v(1 - eps, v))
                u_l = eps if c_lo >= alpha_low else (1-eps if c_hi <= alpha_low else
                    brentq(lambda u: float(cop.conditional_cdf_u_given_v(u, v)) - alpha_low, eps, 1-eps, xtol=1e-5))
                u_h = eps if c_lo >= alpha_high else (1-eps if c_hi <= alpha_high else
                    brentq(lambda u: float(cop.conditional_cdf_u_given_v(u, v)) - alpha_high, eps, 1-eps, xtol=1e-5))
            except Exception:
                u_l = np.nan
                u_h = np.nan
            Q_low.append(float(u_l))
            Q_high.append(float(u_h))
        ...
        fig.add_trace(go.Scatter(
            x=Q_low[np.isfinite(Q_low)].tolist() + Q_high[np.isfinite(Q_high)][::-1].tolist(),
            y=v_grid[np.isfinite(Q_low)].tolist() + v_grid[np.isfinite(Q_high)][::-1].tolist(),
            fill="toself", fillcolor="rgba(0,240,255,0.04)",
            line=dict(width=0), showlegend=False, hoverinfo="skip",
        ))

        fig.add_trace(go.Scatter(
            x=Q_low.tolist(), y=v_grid.tolist(), mode="lines",
            line=dict(color="#ff3355", width=2),
            name=f"Low frontier (α={alpha_low})",
        ))
        fig.add_trace(go.Scatter(
            x=Q_high.tolist(), y=v_grid.tolist(), mode="lines",
            line=dict(color="#00f0ff", width=2),
            name=f"High frontier (α={alpha_high})",
        ))

    # Scatter points colored by signal zone
    if pseudo_u and pseudo_v and len(pseudo_u) > 0:
        u_arr = np.array(pseudo_u)
        v_arr = np.array(pseudo_v)

        if has_cop:
            h_vals = []
            for ui, vi in zip(u_arr, v_arr):
                try:
                    h = float(cop.conditional_cdf_u_given_v(
                        np.clip(ui, 1e-6, 1-1e-6), np.clip(vi, 1e-6, 1-1e-6)))
                    h_vals.append(h)
                except Exception:
                    h_vals.append(0.5)
            ...
            # Zone masks
            low_mask = h_vals < alpha_low
            high_mask = h_vals > alpha_high
            neutral_mask = ~low_mask & ~high_mask

            if np.any(neutral_mask):
                fig.add_trace(go.Scatter(
                    x=u_arr[neutral_mask].tolist(), y=v_arr[neutral_mask].tolist(),
                    mode="markers", marker=dict(size=3, color="rgba(200,220,232,0.25)"),
                    name="Neutral zone", showlegend=True,
                ))
            if np.any(low_mask):
                fig.add_trace(go.Scatter(
                    x=u_arr[low_mask].tolist(), y=v_arr[low_mask].tolist(),
                    mode="markers", marker=dict(size=5, color="#ff3355", opacity=0.8,
                                                 line=dict(width=0.5, color="#fff")),
                    name=f"Signal LOW (h<{alpha_low})",
                ))
            if np.any(high_mask):
                fig.add_trace(go.Scatter(
                    x=u_arr[high_mask].tolist(), y=v_arr[high_mask].tolist(),
                    mode="markers", marker=dict(size=5, color="#00f0ff", opacity=0.8,
                                                 line=dict(width=0.5, color="#fff")),
                    name=f"Signal HIGH (h>{alpha_high})",
                ))
        else:
            fig.add_trace(go.Scatter(
                x=u_arr.tolist(), y=v_arr.tolist(),
                mode="markers", marker=dict(size=3, color="#5a7a90", opacity=0.4),
                name="Pseudo-obs",
            ))

    fig.update_layout(
        **TRON_LAYOUT, height=420,
        title=f"⬡  ARBITRAGE FRONTIERS — {cop_name.upper() if cop_name else 'N/A'}",
        xaxis_title="u (spread 1)", yaxis_title="v (spread 2)",
        xaxis=dict(**TRON_LAYOUT["xaxis"], range=[0, 1]),
        yaxis=dict(**TRON_LAYOUT["yaxis"], range=[0, 1], scaleanchor="x"),
    )
    ...
    dbc.Row([
        ...
        dbc.Col(width=9, children=[
            ...
            dcc.Tabs(id="tabs", value="tab-equity", className="tab-container", children=[
                ...
                dcc.Tab(label="⬡ Copula Diagnostics", value="tab-copula-viz"),
            ]),
...
def render_tab(tab, store):
    ...
    if tab == "tab-copula-viz":
        # Build cycle options from weekly data (only OK cycles with copula data)
        ok_weeks = weekly[weekly.get("status", pd.Series(dtype=str)) == "OK"] if not weekly.empty and "status" in weekly.columns else pd.DataFrame()
        if ok_weeks.empty:
            return dbc.Alert("No OK cycles with fitted copulas to visualize.", className="alert-info",
                             style={"fontFamily":"Share Tech Mono", "letterSpacing":"1px"})

        cycle_options = []
        for _, row in ok_weeks.iterrows():
            cid = int(row.get("cycle", 0))
            pair = row.get("selected_pair", "?")
            cop = row.get("copula", "?")
            t_start = str(row.get("trade_start", ""))[:10]
            cycle_options.append({"label": f"Cycle {cid}: {pair} — {cop} ({t_start})", "value": cid})
        ...
        return html.Div([
            dbc.Row([
                dbc.Col([
                    html.Label("SELECT CYCLE", style={"fontFamily":"Orbitron", "fontSize":"0.7rem",
                                "letterSpacing":"2px", "color":"#00f0ff", "marginBottom":"4px"}),
                    dcc.Dropdown(
                        id="copula-viz-cycle",
                ...
                dbc.Col([
                    html.Div(id="copula-viz-info", style={"paddingTop":"18px"}),
                ], width=6),
            ], style={"marginBottom":"14px"}),
            # Viz row 1: PDF contour + dependency scatter
            dbc.Row([
                dbc.Col(dcc.Graph(id="copula-viz-contour", config={"responsive": True}), width=6),
                dbc.Col(dcc.Graph(id="copula-viz-scatter", config={"responsive": True}), width=6),
            ]),
            # Viz row 2: arbitrage frontiers (full width)
            dbc.Row([
                dbc.Col(dcc.Graph(id="copula-viz-frontiers", config={"responsive": True}), width=12),
            ]),
        ])
...
# Allow dynamic components (cycle selector created inside tab)
app.config.suppress_callback_exceptions = True


@app.callback(
    Output("copula-viz-contour", "figure"),
    Output("copula-viz-scatter", "figure"),
    Output("copula-viz-frontiers", "figure"),
    Output("copula-viz-info", "children"),
    Input("copula-viz-cycle", "value"),
    State("store-results", "data"),
    prevent_initial_call=True,
)
def update_copula_viz(cycle_id, store):
    if not store or cycle_id is None:
        empty = fig_empty("Select a cycle")
    ...
    cycle_data = None
    for w in weekly:
        if int(w.get("cycle", -1)) == int(cycle_id) and w.get("status") == "OK":
            cycle_data = w
            break

    if cycle_data is None:
        empty = fig_empty("Cycle data not found")
        return empty, empty, empty, "Cycle not found"

    cop_name = cycle_data.get("copula", None)
    cop_params = cycle_data.get("copula_params", None)
    pseudo_u = cycle_data.get("pseudo_obs_u", [])
    pseudo_v = cycle_data.get("pseudo_obs_v", [])
    pair = cycle_data.get("selected_pair", "?")

    if not cop_name or cop_params is None:
        empty = fig_empty("No copula fitted")
        return empty, empty, empty, "No copula fitted for this cycle"

    # Build figures
    fig_contour = fig_copula_pdf_contour(cop_name, cop_params, pseudo_u, pseudo_v)
    fig_scatter = fig_dependency_scatter(pseudo_u, pseudo_v, cop_name, cop_params)
    fig_front = fig_arbitrage_frontiers(cop_name, cop_params, pseudo_u, pseudo_v)

    # Info panel
    params_str = ", ".join([f"{p:.4f}" for p in cop_params]) if cop_params else "N/A"
    n_obs = len(pseudo_u)
    info = html.Div([
        html.Span(f"PAIR: ", style={"color":"#5a7a90", "fontFamily":"Share Tech Mono", "fontSize":"0.72rem"}),
        html.Span(f"{pair}", style={"color":"#00f0ff", "fontFamily":"Orbitron", "fontSize":"0.8rem"}),
        html.Span(f"  ·  COPULA: ", style={"color":"#5a7a90", "fontFamily":"Share Tech Mono", "fontSize":"0.72rem"}),
        html.Span(f"{cop_name}", style={"color":"#ff2eed", "fontFamily":"Orbitron", "fontSize":"0.8rem"}),
        html.Span(f"  ·  θ=({params_str})", style={"color":"#5a7a90", "fontFamily":"Share Tech Mono", "fontSize":"0.72rem"}),
        html.Span(f"  ·  N={n_obs}", style={"color":"#00ff88", "fontFamily":"Share Tech Mono", "fontSize":"0.72rem"}),
    ])

    return fig_contour, fig_scatter, fig_front, info
